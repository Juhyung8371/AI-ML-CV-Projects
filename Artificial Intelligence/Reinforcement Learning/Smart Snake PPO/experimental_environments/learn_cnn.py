...
def make_env(_render_mode, map_size):
    ...
    def _init():
        ...
        # frame stacking test code
        # _env = GrayScaleObservation(_env, keep_dim=True)  # need keep_dim for frame stack
...
if __name__ == '__main__':
    ...
    model_lr_low = PPO('CnnPolicy', env_lr_low, verbose=0, batch_size=1024,
    ...
    for i in range(1, 10000):
        model_lr_low.learn(total_timesteps=50000, callback=ScoreCallBack(),
                           reset_num_timesteps=False, tb_log_name="__CNN")
        if i % 4 == 0:
            model_lr_low.save(f"{models_dir}/__CNN{i}")
