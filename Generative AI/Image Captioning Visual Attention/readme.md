...
<img src="readme_images/baseball.jpeg" height="250">
...
I used the COCO captions dataset. This dataset contains images, bounding boxes, labels, and captions from COCO 2014, split into the subsets defined by Karpathy and Li (2015), and takes care of some data quality issues with the original dataset.
...
- Text processing:
  ...
  1. Transform the text captions into integer sequences using the [TextVectorization](https://www.tensorflow.org/api_docs/python/tf/keras/layers/TextVectorization):
      * Override the `standardize` parameter with a custom function to preserve < and > characters for the custom tokens `<start>` and `<end>`.
- Image processing:
...
The decoder uses attention to focus on parts of the input sequence selectively.
...
<img src="readme_images/attention1.png" alt="attention equation 1" height="70">

<img src="readme_images/attention2.png" alt="attention equation 2" height="70">
...
<img src="readme_images/attention3.png" alt="attention equation 3" height="70">
...
The above diagram shows the training model architecture. 

The decoder's job is to generate predictions for the next output token.

1. The decoder receives current word tokens as a batch.
1. It embeds the word tokens to the `ATTENTION_DIM` dimension.
1. The GRU layer keeps track of the word embeddings and returns GRU outputs and states.
...
1. The attention outputs and GRU outputs are added (skip connection) and normalized in a normalization layer.
...
1. In the for loop, we
    ...
    - select Top-K words from logits, and choose a word probabilistically so that we avoid computing softmax over a VOCAB_SIZE-sized vector.
...
Despite the short training time (~ 1 hour), the model can identify and highlight some major components in the images. I noticed that the model worked better with images with less information (patterns, objects, colors, etc.). There are some grammatical errors in the captions, but the meaning is still understandable.
